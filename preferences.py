...
class ErrorEntry(bpy.types.PropertyGroup):
    type: bpy.props.StringProperty(name="type", default="ERROR")
    name: bpy.props.StringProperty(name="name", default="error")
    value: bpy.props.StringProperty(name="value", default="empty")
...
class NodeOSCEnvVarSettings(bpy.types.PropertyGroup):
    ...
    enable_incomming_message_printout: bpy.props.BoolProperty(description="Printout all incomming messages to the info log")
...
def register():
    bpy.utils.register_class(ErrorEntry)
...
def unregister():
    ...
    bpy.utils.unregister_class(ErrorEntry)
